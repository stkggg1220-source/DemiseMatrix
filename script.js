...
document.getElementById('export-btn').addEventListener('click', function() {
  ...
  // スマホなどでの表示崩れを防ぐため、一時的にスクロールをリセット
  window.scrollTo(0, 0);

  // html2canvasを実行
  html2canvas(targetElement, {
    backgroundColor: '#121212', // サイトの背景色
    scale: 2,                   // 高画質化
    useCORS: true               // 外部画像エラー防止
  }).then(canvas => {
    // 画像データをURL（Base64形式）に変換
    const imageURL = canvas.toDataURL("image/png");

    // 1. 空の新しいタブを開く
    const newTab = window.open();

    // 2. もしポップアップブロック等で開けなかった場合の処理
    if (!newTab) {
      alert("ポップアップがブロックされました。ブラウザの設定を確認してください。");
      return;
    }

    // 3. 新しいタブの中に、画像を中央配置で表示するHTMLを書き込む
      ...
      <head>
        ...
        <style>
          body {
            margin: 0;
            padding: 20px;
            background-color: #121212; /* 元のサイトに合わせたダーク背景 */
            display: flex;
